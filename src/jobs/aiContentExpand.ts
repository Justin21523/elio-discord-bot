...
async function generateGreetings() {
  try {
    ...
    if (!result.ok) {
      logger.error("[JOB:ContentExpand] Greetings generation failed", {
        error: result.error,
      });
      return result;
    }
    ...
    logger.error("[JOB:ContentExpand] Greetings generation error", {
...
async function generateMedia() {
  try {
    ...
    if (!result.ok) {
      logger.error("[JOB:ContentExpand] Media generation failed", {
        error: result.error,
      });
      return result;
    }
    ...
    logger.error("[JOB:ContentExpand] Media generation error", {
      error: getErrorMessage(error),
      stack: error instanceof Error ? error.stack : undefined,
    });
    return {
...
async function generateScenarios() {
  try {
    ...
    if (!result.ok) {
      logger.error("[JOB:ContentExpand] Scenarios generation failed", {
        error: result.error,
      });
      return result;
    }
    ...
    logger.error("[JOB:ContentExpand] Scenarios generation error", {
      error: getErrorMessage(error),
      stack: error instanceof Error ? error.stack : undefined,
    });
    return {
 ...
 */
async function upsertGreetings(items: Array<{ text: string; tags?: string[] }>) {
...
async function upsertMedia(
  items: Array<{ url: string; caption?: string; tags?: string[]; type?: string }>
) {
 ...
 * @returns {Promise<number>} - Number of items upserted
 */
async function upsertScenarios(items: any[]) {
 ...
 */
export async function run(_client: any) {
  try {
    ...
    logger.error("[JOB:ContentExpand] Error", {
      error: getErrorMessage(error),
      stack: error instanceof Error ? error.stack : undefined,
    });
