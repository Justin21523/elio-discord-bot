 ...
 */
export async function runDynamicDataUpdate(ai: any) {
  try {
 ...
 */
async function analyzeRAGResources(ai: any) {
  try {
    // Get existing personas from database
    const existingPersonas = await withCollection("personas", (col) =>
      col.find({}, { projection: { name: 1 } }).toArray()
    );
    const existingNames = new Set((existingPersonas as any[]).map((p) => p.name));
  ...
  "scenarios": [{"description": "...", "characters": [...]}]
    ...
    for (const file of characterFiles) {
      ...
      if (nameMatch) {
        ...
        if (characterName && !existingNames.has(characterName)) {
          newCharacters.push({
            name: characterName,
    ...
    logger.error("[DYNAMIC-UPDATE] RAG analysis failed", { error: getErrorMessage(error) });
 ...
 */
async function findCharacterFiles(dir: string): Promise<string[]> {
  const files: string[] = [];
  try {
    ...
    logger.debug("[DYNAMIC-UPDATE] Error reading directory", { dir, error: getErrorMessage(error) });
  }
  return files;
}
 ...
 */
async function generateNewPersonas(ai: any, newCharacters: any[]): Promise<any[]> {
  const personas: any[] = [];
 ...
 */
async function generateNewScenarios(ai: any, ragAnalysis: any): Promise<any[]> {
  const scenarios: any[] = [];

  try {
    // Get existing personas for context
    const personas = await withCollection("personas", (col) =>
      col.find({}, { projection: { name: 1 } }).limit(15).toArray()
    );
    const personaNames = (personas as any[]).map((p) => p.name).join(", ");
 ...
 */
async function generateNewGreetings(ai: any, ragAnalysis: any): Promise<any[]> {
  const greetings: any[] = [];
 ...
 */
async function insertPersonas(personas: any[]) {
 ...
 */
async function insertGreetings(greetings: any[]) {
