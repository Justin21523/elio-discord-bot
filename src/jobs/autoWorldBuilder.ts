...
export async function run(client: any) {
  try {
    ...
    if (!selectedType) {
      logger.warn("[JOB] Failed to select lore type");
      return;
    }
...
Style: Engaging, imaginative, suitable for all ages`;
    ...
    const moderationResult = await ai.moderation.scan({ content: loreText });
    if (!moderationResult.ok) {
      logger.warn("[JOB] Moderation scan failed", { error: moderationResult.error });
      return;
    }
    if (moderationResult.data.flagged) {
      logger.warn("[JOB] Generated lore failed moderation", {
        categories: moderationResult.data.categories,
      });
      return;
    }
    ...
    if (["character", "location", "species"].includes(selectedType)) {
      ...
      const searchResult = await ai.web.search(imageSearchQuery, { top: 10 });
      const results = searchResult.ok
        ? (searchResult.data?.data?.results ?? searchResult.data?.results ?? [])
        : [];
    ...
    if (personas.length === 0) {
      logger.warn("[JOB] No personas found for curator");
      return;
    }
    const curator = personas[Math.floor(Math.random() * personas.length)];
    if (!curator) {
      logger.warn("[JOB] Failed to select curator persona");
      return;
    }
    ...
    for (const channelId of config.channelIds) {
      try {
        ...
        await webhooks.sendAsPersona(
          ...
          {
            name: curator.name,
            avatar: curator.avatarUrl || curator.avatar,
            color: curator.color,
          },
    ...
    logger.error("[JOB] autoWorldBuilder failed", { error: getErrorMessage(error) });
  }
}
...
async function getAutoWorldBuilderConfig() {
  try {
    ...
    let config = (await configCol.findOne({ key: "auto_world_builder" })) as any;
 ...
 */
export async function updateConfig(updates: Partial<AutoWorldBuilderConfig>) {
  try {
    const configCol = getCollection("bot_config");
