...
import {
  getUnexportedInteractions,
  getQualityInteractions,
  markExported,
  getStats,
...
async function exportInteractionsToJSONL({ qualityOnly = false, limit = 1000, dryRun = false } = {}) {
  ...
  try {
    ...
    if (!dryRun) {
      const ids = (interactions as any[]).map((i) => String(i._id));
    ...
    if (statsResult.ok) {
      logger.info("[ExportInteractions] Current statistics", { stats: statsResult.data });
    }
    ...
    logger.error("[ExportInteractions] Export failed", {
...
async function dailyExportJob() {
  ...
  if (result.ok) {
    logger.info("[ExportInteractions] Daily export completed", { data: result.data });
  } else {
    logger.error("[ExportInteractions] Daily export failed", { error: result.error });
  }
