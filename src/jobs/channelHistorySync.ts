...
type JobRunSummary = {
  ...
  totalFetched: number;
  totalStored: number;
  errors: Array<{ guildId: string; guildName: string; error: string }>;
};
 ...
 */
async function getLastSyncTime(db: any, guildId: string) {
 ...
 */
async function updateLastSyncTime(db: any, guildId: string) {
 ...
 */
async function getGuildSyncConfig(db: any, guildId: string) {
 ...
 */
async function runJob(client: any, db: any, aiClient: any, config: any) {
  console.log(`[${JOB_NAME}] Job started at ${new Date().toISOString()}`);

  const results: JobRunSummary = {
    startTime: new Date(),
    guilds: [],
    totalFetched: 0,
    totalStored: 0,
  ...
  for (const guild of client.guilds.cache.values()) {
    try {
      const result: any = await syncGuild(client, db, aiClient, guild, config);
 ...
 */
export function createChannelHistorySyncJob(
  client: any,
  db: any,
  aiClient: any,
  config: ChannelHistorySyncConfig = {}
) {
  ...
  const job: any = cron.schedule(cronSchedule, async () => {
    try {
 ...
 */
export async function runOnceSync(client: any, db: any, aiClient: any, options: any = {}) {
  const config = {
    maxDays: parseInt(process.env.CHANNEL_HISTORY_MAX_DAYS || "7", 10),
    ...options,
  };
