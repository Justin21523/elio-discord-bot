...
Respond with JSON:
{
  ...
  "response_tone": "helpful/funny/curious/concerned"
        ...
        const persona = personas.find(
          (p: any) => p.name.toLowerCase() === decision.persona.toLowerCase()
        );
...
Use your knowledge naturally - don't explicitly cite sources.`;
    ...
    logger.error("[JOB] autoPersonaChat failed", { error: getErrorMessage(error) });
  }
}
...
async function getAutoPersonaChatConfig() {
  try {
    ...
    let config = (await configCol.findOne({ key: "auto_persona_chat" })) as any;
 ...
 */
export async function updateConfig(updates: Partial<AutoPersonaChatConfig>) {
  try {
    const configCol = getCollection("bot_config");
