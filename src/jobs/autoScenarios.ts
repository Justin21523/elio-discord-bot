...
Format your response as JSON:
{
  ...
  try {
    ...
    logger.error("[JOB:AutoScenarios] Generation error", {
...
export async function run(client: any) {
  try {
    ...
    if (!guild) {
      logger.warn("[JOB:AutoScenarios] No guild found");
      return;
    }
    ...
    // Get host persona
    const hostPersonaRes = await personas.getPersona(scenario.hostPersonaName || "Elio");
    if (!hostPersonaRes.ok) {
      logger.warn("[JOB:AutoScenarios] Host persona not found", {
        persona: scenario.hostPersonaName,
        error: hostPersonaRes.error,
      });
      return;
    }
    const hostPersona = hostPersonaRes.data;
    ...
    logger.info(`[JOB:AutoScenarios] ✅ Posted scenario to ${channel.name}`);
  } catch (error) {
    logger.error("[JOB:AutoScenarios] Error", {
      error: getErrorMessage(error),
      stack: error instanceof Error ? error.stack : undefined,
    });
