  ...
  try {
    ...
    if (updates.length > 0) {
      try {
        ...
        logger.error("[JOB] Failed to send bandit update", { error: getErrorMessage(error) });
      }
    }
    ...
    logger.error("[JOB] updateBanditWeights failed", { error: getErrorMessage(error) });
  }
}
 ...
 */
async function sendBanditUpdate(updates: any[]) {
  try {
    ...
    logger.error("[JOB] sendBanditUpdate failed", { error: getErrorMessage(error) });
