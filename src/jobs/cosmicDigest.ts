...
export async function run(client: any) {
  try {
    ...
    // Get Elio persona
    const elioPersonaRes = await personas.getPersona("Elio");
    if (!elioPersonaRes.ok) {
      logger.warn("[JOB:CosmicDigest] Elio persona not found", {
        error: elioPersonaRes.error,
      });
      return;
    }
    const elioPersona = elioPersonaRes.data;
    ...
    if (!rewriteResult.ok) {
      logger.error("[JOB:CosmicDigest] LLM rewrite failed", {
    ...
    if (!guild) {
      logger.warn("[JOB:CosmicDigest] No guild found");
      return;
    }
    ...
    logger.error("[JOB:CosmicDigest] Error", {
