...
export async function run(client: any) {
  try {
    ...
    for (const guild of guilds.values()) {
      ...
      const channelIdsRaw = config.config.channelIds;
      const channelIds: string[] = Array.isArray(channelIdsRaw)
        ? channelIdsRaw.filter(
            (id: unknown): id is string => typeof id === "string" && id.length > 0
          )
        : [];
      if (channelIds.length === 0) {
        logger.debug(
          `[JOB:ChannelSummary] No valid channel IDs for guild ${guild.id}`
        );
        continue;
      }
    ...
    logger.error("[JOB:ChannelSummary] Error", {
