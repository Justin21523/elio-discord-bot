 ...
 */
function matchesChannelPattern(channelName, pattern) {
 ...
 */
function shouldProcessServer(guildId, config) {
 ...
 */
async function findPatternMatchedChannels(client, config) {
 ...
 */
async function isAlreadyShared(db, url, guildId) {
 ...
 */
async function recordShare(db, shareData) {
  const collection = db.collection(COLLECTION_NAME);
...
async function checkRelevance(item) {
  try {
    ...
    return result.data.text.trim().toUpperCase().includes("RELEVANT");
  } catch (error) {
    logger.warn("[socialMediaMonitor] Relevance check failed, defaulting to relevant", { error: error.message });
    return true;
  }
}
 ...
 */
async function generateCharacterReaction(persona, item) {
  try {
    ...
    logger.error("[socialMediaMonitor] Error generating character reaction", { error: error.message });
    return null;
  }
}
 ...
 */
async function searchAllSources(config) {
  const results = [];
  ...
  const randomKeywords = [
    ...
    SEARCH_KEYWORDS.primary[Math.floor(Math.random() * SEARCH_KEYWORDS.primary.length)],
    SEARCH_KEYWORDS.primary[Math.floor(Math.random() * SEARCH_KEYWORDS.primary.length)],
    // Include one from each secondary category
    SEARCH_KEYWORDS.characters[Math.floor(Math.random() * SEARCH_KEYWORDS.characters.length)],
    SEARCH_KEYWORDS.topics[Math.floor(Math.random() * SEARCH_KEYWORDS.topics.length)],
    SEARCH_KEYWORDS.timely[Math.floor(Math.random() * SEARCH_KEYWORDS.timely.length)],
    SEARCH_KEYWORDS.production[Math.floor(Math.random() * SEARCH_KEYWORDS.production.length)],
  ];
  ...
  for (const query of randomKeywords.slice(0, 4)) {
    try {
      ...
      logger.warn(`[socialMediaMonitor] Search failed for query: ${query}`, { error: error.message });
    }
  }
...
async function selectRandomPersona() {
  try {
    ...
    if (!result.ok || !result.data || result.data.length === 0) {
      logger.warn("[socialMediaMonitor] No personas available", {
        ok: result?.ok,
    ...
    logger.error("[socialMediaMonitor] Error selecting persona", { error: error.message });
    return null;
  }
}
  ...
  try {
    ...
    for (const channelId of config.channelIds) {
      try {
        ...
        logger.warn(`[socialMediaMonitor] Could not fetch channel ${channelId}: ${err.message}`);
      }
    }
      ...
      try {
        ...
        for (const item of allResults) {
          ...
          try {
            ...
            logger.error("[socialMediaMonitor] Failed to post", { error: postError.message });
          }
        }

        logger.info(`[socialMediaMonitor] Shared ${sharesThisRun} items in ${channel.name}`);
      } catch (channelError) {
        logger.error(`[socialMediaMonitor] Error processing channel ${channelId}`, {
    ...
    logger.error("[JOB:SocialMediaMonitor] Error:", { error: error.message, stack: error.stack });
