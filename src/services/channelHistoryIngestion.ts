...
export class ChannelHistoryIngestion {
  ...
  constructor(db: { collection: (name: string) => any }, aiClient: any = null) {
    this.db = db;
  ...
  async ingestChannel(
    ...
    try {
      ...
      while (hasMore) {
        ...
        const fetchOptions: Record<string, any> = { limit: BATCH_SIZE };
        if (lastMessageId) {
        ...
        const messageArray = (Array.from(messages.values()) as any[]).filter(
          (msg: any) => msg.createdAt >= cutoffDate
        );
  ...
  async _processBatch(
    ...
    for (const msg of messages) {
      ...
      const doc = {
        ...
        attachments: Array.from(msg.attachments.values() as any).map((a: any) => ({
          id: a.id,
          url: a.url,
          name: a.name,
          contentType: a.contentType,
        })),
        embeds: (msg.embeds as any[]).map((e: any) => ({
          title: e.title,
    ...
    if (generateEmbeddings && this.aiClient && stored > 0) {
      await this._generateEmbeddings(documents.filter((d: any) => !d.optedOut));
    }
    ...
    * @private
    */
  async _generateEmbeddings(documents: any[]) {
    if (!this.aiClient) return;

    try {
      const texts = documents.map((d: any) => d.cleanContent || d.content);
      ...
      if (response.ok && response.data.embeddings) {
        ...
        const bulkOps = documents.map((doc: any, i: number) => ({
          updateOne: {
            filter: { messageId: doc.messageId },
            update: {
              $set: {
   ...
   * @param {object} options - Ingestion options
   */
  async ingestChannels(channels: any[], options: any = {}) {
    const results: any[] = [];
    ...
    const summary = {
      ...
      totalFetched: results.reduce((sum: number, r: any) => sum + (r.data?.totalFetched || 0), 0),
      totalStored: results.reduce((sum: number, r: any) => sum + (r.data?.totalStored || 0), 0),
      results,
    };
   ...
   */
  async ingestGuild(guild: any, options: any = {}) {
    const { excludeChannels = [], includeChannels = null, ...ingestionOptions } = options;

    try {
      // Get all text channels
      let channels = guild.channels.cache
        .filter((c: any) => c.type === 0) // GUILD_TEXT
        .filter((c: any) => !excludeChannels.includes(c.id));

      // Filter to specific channels if provided
      if (includeChannels) {
        channels = channels.filter((c: any) => includeChannels.includes(c.id));
      }

      // Check bot permissions for each channel
      const accessibleChannels = channels.filter((c: any) => {
    ...
    */
  async getStats(guildId: string | null = null) {
    try {
      ...
      const result = (await this.db.collection(COLLECTION).aggregate(pipeline).toArray()) as any[];
  ...
  async getMessagesByChannel(
    channelId: string,
    options: { limit?: number; skip?: number; includeOptedOut?: boolean } = {}
  ) {
   ...
   */
  async getRecentContext(channelId: string, limit = 20, before: Date | null = null) {
    try {
    ...
    */
  async getTrainingEligibleMessages(
    options: { limit?: number; minLength?: number; guildId?: string | null } = {}
  ) {
