 ...
 */
function getCharacterRelationships(personaName: string): string {
 ...
 */
function wrapForDiscordChat(personaPrompt: string, personaName: string): string {
...
export function isLlamaEnabled(): boolean {
  return Boolean(USE_LLAMA_SERVER && LLAMA_SERVER_URL);
}
 ...
 * @returns {Promise<{ok: true, data: LlamaResponse} | {ok: false, error: object}>}
 */
export async function generateElioReply(
  userMessage: string,
  conversationHistory: ConversationMessage[] = [],
  options: PersonaReplyOptions = {}
): Promise<any> {
 ...
 * @returns {string}
 */
function formatHistoryForAnalysis(history: ConversationMessage[]): string {
