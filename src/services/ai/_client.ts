...
export async function httpPostJson(path: string, body: unknown, timeoutMs = AI_SERVICE_TIMEOUT_MS) {
  if (AI_MOCK_MODE) {
    return mockFetch(path, body as any, 'POST');
  }
...
export async function httpPostForm(path: string, formData: any, timeoutMs = AI_SERVICE_TIMEOUT_MS) {
  if (AI_MOCK_MODE) {
    const payload =
...
export async function httpGet(path: string, timeoutMs = AI_SERVICE_TIMEOUT_MS) {
  if (AI_MOCK_MODE) {
