 ...
 */
export async function batchUpdateBandit(updates: any[]): Promise<any> {
  // `updates` is an array of { arm, reward, messageId }-like objects
 ...
 */
export async function updateBandit(arm: string, reward: number): Promise<any> {
 ...
 * @returns {object} Result
 */
export async function resetBandit(arm: string | null = null): Promise<any> {
 ...
 * @returns {object} Result
 */
export async function replyWithStrategy(
  strategy: string,
  {
    persona,
    message,
    history = [],
    maxLen = 80,
  }: { persona: string; message: string; history?: any[]; maxLen?: number }
): Promise<any> {
