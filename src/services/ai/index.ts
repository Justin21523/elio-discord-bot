...
export async function classifyRelevance(text: string, vlmCaption: string | null = null) {
...
export async function personaCompose({
  personaId,
  text,
  ragSnippets = [],
  vlmCaption = null,
  ...
  let prompt = `Persona ${personaId} responds to: ${text}`;
...
export async function summarizeNewsPersona(query: string, personaVoice: string) {
