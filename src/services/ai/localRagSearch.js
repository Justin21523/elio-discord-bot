...
function loadRagDocuments() {
  ...
  if (!fs.existsSync(RAG_RESOURCES_DIR)) {
    logger.warn("[LocalRAG] RAG resources directory not found:", RAG_RESOURCES_DIR);
    return ragDocuments;
  }
  ...
  for (const file of files) {
    try {
      ...
      logger.warn(`[LocalRAG] Failed to load ${file}:`, err.message);
    }
  }
 ...
 * @returns {number}
 */
function calculateRelevance(queryKeywords, docKeywords) {
 ...
 */
export function getRagContext(query, personaName = null) {
 ...
 * @returns {boolean}
 */
export function isLoreQuery(query) {
  const loreKeywords = [
