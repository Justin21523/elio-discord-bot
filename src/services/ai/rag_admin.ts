...
export async function listDocs(namespace?: string): Promise<any> {
...
export async function deleteDoc(doc_id: string): Promise<any> {
...
export async function disableChunk(chunk_id: string, disabled = true): Promise<any> {
...
export async function importIndex(dump: unknown): Promise<any> {
...
export async function exportData(
  { limit = 200, format = 'json' }: { limit?: number; format?: string } = {}
): Promise<any> {
  const payload = {
...
export async function purge(
  {
    ...
    source,
    confirm = false,
  }: { olderThanDays?: number; source?: string; confirm?: boolean } = {}
): Promise<any> {
  const payload = {
    index: { name: RAG_INDEX_NAME, provider: 'atlas' },
