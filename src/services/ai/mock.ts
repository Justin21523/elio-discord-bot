...
const short = (text: unknown = "", limit = 120) =>
  (text || "").toString().slice(0, limit).trim() || "mocked content";

const mockVector = (len = 3) =>
  Array.from({ length: len }, (_, i) => 0.01 * (i + 1));
...
export async function mockPost(
  ...
  switch (endpoint) {
    ...
    case "/llm/summarizeNews": {
      const topics: string[] = Array.isArray(payload.topics) ? payload.topics : ["news"];
    ...
    case "/story/dialogue": {
      const characters: string[] = Array.isArray(payload.characters) ? payload.characters : ["A", "B"];
    ...
    case "/moderation/batch-scan":
      const moderationTexts: any[] = Array.isArray(payload.texts) ? payload.texts : [];
      return {
        ok: true,
        data: {
          results: moderationTexts.map((t) => ({ text: t, safe: true, flags: [] })),
        },
      };
    ...
    case "/agent/multi-task":
      ...
      return {
        ...
        data: {
          ...
          failed: 0,
          results: tasks.map((t, idx) => ({ task: t, status: "done", idx })),
    ...
    case "/agent/persona-challenge":
      ...
      return {
        ...
        data: {
          persona: payload.persona_name || "MockPersona",
          replies: messages.slice(0, payload.max_replies || 3).map((m, idx) => ({
            text: `Mock reply ${idx + 1} to "${short(m.content || m, 40)}"`,
          })),
          total_evaluated: messages.length || 0,
        },
      };
