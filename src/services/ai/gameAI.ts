...
export async function battleAction({
  ...
  cooldowns?: Record<string, unknown> | null;
 ...
 */
export async function battleEnd(sessionId: string): Promise<any> {
 ...
 */
export async function battleStats(sessionId: string): Promise<any> {
...
export async function reactionTime({
  ...
  isCritical = false,
  fatigueTurns = 0,
}: { skillLevel?: number; isCritical?: boolean; fatigueTurns?: number }): Promise<any> {
 ...
 */
export async function adventureVote({
  choices,
  personaTraits = {},
  context = "",
}: {
