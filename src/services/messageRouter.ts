...
const activeConversations = new Map<string, { personaName: string; expiresAt: number }>();
 ...
 */
export async function handlePublicMessage(message: any, services: any) {
  try {
 ...
 */
async function detectExplicitPersonaMention(messageContent: string): Promise<string | null> {
 ...
 */
async function shouldAutoReply(message: any, config: any, services: any): Promise<any> {
  ...
  if (message.reference) {
    try {
      ...
      logger.debug("[AUTO-REPLY] Failed to fetch replied message", { error: getErrorMessage(error) });
  ...
  */
async function pickRandomPersona(): Promise<string> {
  try {
    ...
    logger.error("[AUTO-REPLY] Failed to pick random persona", { error: getErrorMessage(error) });
  }
  ...
  */
async function generateOriginalAssistantReply(message: any, services: any, config: any): Promise<any | null> {
  try {
    const { ai } = services;
    ...
    if (config.useRAG && ai.rag && message.content.length > 20) {
      try {
        ...
        if (ragResult.ok && ragResult.data.hits?.length > 0) {
          context = (ragResult.data.hits as any[])
            .slice(0, 3)
            .map((hit: any) => hit.chunk.substring(0, 200))
            .join("\n");
          usedRAG = true;
        }
      } catch (error) {
        logger.debug("[AUTO-REPLY] RAG search failed (non-critical)", { error: getErrorMessage(error) });
      }
    }
    ...
    if (config.useVLM && message.attachments.size > 0 && ai.vlm) {
      try {
        const attachment = Array.from(message.attachments.values() as any).find((a: any) =>
        ...
        logger.warn("[AUTO-REPLY] VLM failed", { error: getErrorMessage(error) });
      }
    }
...
Provide a helpful, friendly response (1-3 sentences).`;
  ...
  */
async function generateSmartReply(message: any, decision: any, services: any, config: any): Promise<any | null> {
  try {
    const { ai } = services;
    ...
    // 2. RAG - ONLY if no conversation history (to reduce latency)
      ...
      try {
        ...
        if (ragResult.ok && ragResult.data.hits?.length > 0) {
          ...
          context += "\n" + (ragResult.data.hits as any[])
            .slice(0, 2)
            .map((hit: any) => hit.chunk.substring(0, 150)) // Reduced from 250 to 150 chars
            .join("\n");
          usedRAG = true;
    ...
    if (ai?.personaLogic) {
      try {
        ...
        logger.warn("[AUTO-REPLY] Persona logic service failed, falling back", { error: getErrorMessage(error) });
      }
    }
 ...
 */
function cleanResponseText(text: string, personaName: string) {
  if (!text) return text;
...
function buildLogicOnlyReply(userMessage: string, persona: any) {
...
// Cooldown helpers
function isOnCooldown(key: string, duration: number) {
