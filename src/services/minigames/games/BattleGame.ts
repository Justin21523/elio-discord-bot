...
export class BattleGame extends BaseGame {
  ...
  makeFighter(user: any, isBot = false, difficulty = 'normal') {
    const template =
      DEFAULT_PLAYERS[Math.floor(Math.random() * DEFAULT_PLAYERS.length)] ??
      DEFAULT_PLAYERS[0]!;
    const fighter: any = {
      userId: user.id,
    ...
    if (isBot) {
      ...
      fighter.skillLevel = (BOT_SKILL_LEVELS as Record<string, number>)[difficulty] || 0.6;
  ...
  async botTurn(bot: any) {
    ...
    if (AI_ENABLED && ai?.gameAI && bot.aiInitialized) {
      try {
        ...
        const availableActions = (bot.skills as string[]).filter(
          (s: string) => (this.gameData.cooldowns.get(`${bot.userId}_${s}`) || 0) === 0
        );
    ...
    if (!skillId) {
      skillId =
        (bot.skills as string[]).find(
          (s: string) => (this.gameData.cooldowns.get(`${bot.userId}_${s}`) || 0) === 0
        ) || (bot.skills as string[])[0];
    }
  ...
  async sendState(prefix: string) {
    ...
    const current = this.currentPlayer();
    const buttons = (current.skills as string[]).map((sid: string) =>
      new ButtonBuilder()
        .setCustomId(`battle_skill_${this.sessionId}_${sid}`)
        .setLabel(String(SKILLS[sid]?.name ?? sid))
  ...
  async generateFlavor(base: string) {
