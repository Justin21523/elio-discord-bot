...
export class AdventureGame extends BaseGame {
  ...
  async sendNode() {
    ...
    const buttons = (node.choices as any[]).map((choice: any, idx: number) => {
  ...
  async generateFlavor(baseText: string) {
    ...
    try {
      ...
      logger.debug("[ADVENTURE] Markov flavor failed", { error: msg });
    }
    return "";
  }

  applyReward(points: number) {
    for (const p of this.players) {
      p.score = (p.score || 0) + points;
    }
  }

  applyPenalty(points: number) {
    for (const p of this.players) {
      p.score = (p.score || 0) - points;
    }
  }
