...
export class KeywordPMIGame extends BaseGame {
  ...
  buildTokens() {
    ...
    return (sourceData.documents as any[])
      .map((d: any) =>
        (d.text || "")
          .toLowerCase()
          .replace(/[^a-z0-9\s]/g, " ")
  ...
  async nextRound() {
    ...
    this.gameData.options = this.pickOptions(this.target);
    await this.channel.send({
      embeds: [
        {
  ...
  pickToken() {
    return this.tokens[Math.floor(Math.random() * this.tokens.length)] ?? "communiverse";
  }

  pickOptions(target: string) {
    ...
    const options = Array.from(distractors) as string[];
  ...
  async handleAction(userId: string, action: string, data: any = {}) {
    ...
    const options = (this.gameData.options || []) as string[];
    ...
    const best = this.bestOption(this.target, options);
  ...
  bestOption(target: string, options: string[]) {
    let best = options[0];
