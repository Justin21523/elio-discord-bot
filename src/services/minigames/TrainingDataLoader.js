...
export async function loadAll() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      logger.error(`[TrainingDataLoader] Error loading ${file}:`, error.message);
    }
  }
 ...
 */
export function getRandomTrivia(count = 5, character = null) {
 ...
 * @returns {Array} Array of documents
 */
export function getCharacterDocuments(character) {
  if (!irDocuments || irDocuments.length === 0) {
    return [];
  }
...
function truncateAnswer(text, maxLength = 100) {
  const cleaned = cleanText(text);
