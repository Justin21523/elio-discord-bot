...
export class MessageArchive {
  ...
  async getByAuthor(
    authorId: string,
    options: { guildId?: string | null; limit?: number; skip?: number } = {}
  ) {
  ...
  async getByTimeRange(
    ...
    options: { guildId?: string | null; channelId?: string | null; limit?: number } = {}
  ) {
    ...
    */
  async getChannelActivity(channelId: string, days = 7) {
    try {
    ...
    */
  async getTopAuthors(
    options: { guildId?: string | null; channelId?: string | null; days?: number; limit?: number } = {}
  ) {
  ...
  async getRAGContext(
    channelId: string,
    query: string,
    options: { recentLimit?: number; semanticLimit?: number; maxTokens?: number } = {}
  ) {
    const {
      recentLimit = 10,
      semanticLimit = 5,
    ...
    try {
      ...
      const recentMessages = (recentResult as any[]).reverse();
      ...
      if (this.aiClient && query) {
        ...
        if (semanticResult.ok) {
          ...
          const recentIds = new Set(recentMessages.map((m: any) => m.messageId));
          semanticMessages = (semanticResult.data as any[]).filter(
            (m: any) => !recentIds.has(m.messageId)
          );
        }
      }
      ...
      const context = {
        ...
        relevant: semanticMessages.map((m: any) => ({
          author: m.authorName || m.authorTag,
          content: m.cleanContent || m.content,
          timestamp: m.timestamp,
  ...
  async exportForTraining(
    ...
    try {
      ...
      if (format === 'jsonl') {
        const jsonl = filtered.map((item: any) => JSON.stringify(item)).join('\n');
  ...
  async getById(messageId: string) {
    ...
    */
  async count(
    options: { guildId?: string | null; channelId?: string | null; authorId?: string | null } = {}
  ) {
