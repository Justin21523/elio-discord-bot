...
const ACTION_DELTA: Record<string, any> = {
...
export async function meet({ guildId, channelId, persona }: any): Promise<Result<any>> {
  try {
    ...
    logError("[persona.meet] failed", { error: getErrorMessage(cause), guildId, channelId, persona });
...
export async function act({ guildId, userId, persona, action }: any): Promise<Result<any>> {
  try {
    ...
    logError("[persona.act] failed", { error: getErrorMessage(cause), guildId, userId, persona, action });
...
export async function getPersonaConfig({ guildId, persona }: any): Promise<Result<any>> {
  try {
...
/** Patch config (admin). */
export async function setPersonaConfig({ guildId, persona, patch }: any): Promise<Result<any>> {
  try {
    const key = { guildId, persona: persona?.id || persona?.name || "default" };
    const now = new Date();
...
export async function getConfig(guildId: string): Promise<Result<any>> {
  try {
...
export async function setConfig(guildId: string, updates: Record<string, any>): Promise<Result<any>> {
  try {
    const now = new Date();
...
export async function keywordTriggersEnabled(guildId: string): Promise<Result<{ enabled: boolean }>> {
  try {
...
function affinityLevel(sum: number) {
 ...
 */
async function getAndBumpCooldown({ guildId, userId, persona, action }: any): Promise<Result<any>> {
  const key = {
    guildId,
    userId,
...
export async function getPersona(name: string): Promise<Result<any>> {
  try {
...
export async function listPersonas(): Promise<Result<any[]>> {
  try {
...
export async function affinityDelta({ guildId, userId, personaId, delta, action }: any): Promise<Result<any>> {
  try {
    ...
    logError("[persona.affinityDelta] failed", { error: getErrorMessage(cause), guildId, userId, personaId, action });
