 ...
 * Result helpers
 */
function ok<T>(data: T): Promise<Result<T>> { return Promise.resolve({ ok: true, data }); }
function err(code: string, message: string, details?: unknown): Promise<Result<never>> {
 ...
 */
const DEFAULT_THRESHOLDS: number[] = [0, 10, 30, 60, 100, 160, 230, 320, 430, 560, 700];

/**
 * Internal: compute level from points and thresholds.
 */
function computeLevel(points: number, thresholds: number[] = DEFAULT_THRESHOLDS): number {
  let lvl = 0;
  for (let i = 0; i < thresholds.length; i++) {
    const threshold = thresholds[i];
    if (threshold === undefined) break;
    if (points >= threshold) lvl = i;
    else break;
  }
  return lvl;
}
 ...
 */
async function loadThresholdsForGuild(_guildId: string): Promise<number[]> {
 ...
 */
async function _ensureProfile(guildId: string, userId: string): Promise<any> {
 ...
 */
export async function award(guildId: string, userId: string, delta: number): Promise<Result<{ points: number; level: number }>> {
 ...
 */
export async function getProfile(guildId: string, userId: string): Promise<Result<any>> {
 ...
 */
export async function leaderboard({ guildId, limit = 10 }: { guildId: string; limit?: number }): Promise<Result<{ entries: any[]; total: number }>> {
  ...
  try {
    ...
    const entries = (docs as any[]).map((d: any, index: number) => ({
      rank: index + 1,
      userId: d.userId,
      points: d.points || 0,
      level: d.level || 0,
 ...
 */
export async function currentLevel(points: number): Promise<Result<number>> {
  try {
 ...
 */
export async function setPoints(guildId: string, userId: string, points: number): Promise<Result<{ points: number; level: number }>> {
 ...
 */
export async function seasonalReset(guildId: string): Promise<Result<{ matched: number; modified: number }>> {
