...
import type { InteractionResponseSource } from '../db/models/interaction.js';
import { logger } from '../util/logger.js';

type InteractionLogParams = {
  guildId: string;
  channelId: string;
  userId: string;
  username: string;
  ...
  userMessage: string;
  botResponse: string;
  responseSource?: InteractionResponseSource;
  similarity?: number | null;
  strategy?: string | null;
};

type QueuedLog = InteractionLogParams & {
  responseSource: InteractionResponseSource;
  similarity: number | null;
  strategy: string | null;
};

type ResultLike = { ok: boolean; data?: unknown; error?: string };
...
class InteractionLogger {
  ...
  async init(): Promise<this> {
    ...
    logger.info('[InteractionLogger] Initialized', { batchSize: this.batchSize });
    return this;
  }
  ...
  log({
    ...
    ok: true;
    data: { queued: boolean; reason?: string };
  } {
    if (!this.enabled) {
  ...
  async logImmediate({
    ...
    similarity = null,
    strategy = null,
  }: InteractionLogParams): Promise<ResultLike> {
    if (!this.enabled) {
    ...
    try {
      ...
      logger.error('[InteractionLogger] Immediate log error', { error: msg });
  ...
  async flush() {
    ...
    try {
      ...
      if (failed > 0) {
        logger.warn('[InteractionLogger] Batch flush partial failure', {
          successful,
          failed,
        });
      } else {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      logger.error('[InteractionLogger] Flush error', { error: msg });
  ...
  async recordUserFeedback(
    userId: string,
    feedback: { rating?: number; thumbsUp?: boolean; thumbsDown?: boolean }
  ): Promise<ResultLike> {
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      logger.error('[InteractionLogger] Feedback error', { error: msg });
      return { ok: false, error: msg };
    }
  }
  ...
  async recordFeedbackById(
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      logger.error('[InteractionLogger] Feedback by ID error', { error: msg });
      return { ok: false, error: msg };
    }
  }
  ...
  async getStatistics() {
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      logger.error('[InteractionLogger] Stats error', { error: msg });
      return { ok: false, error: msg };
    }
  }
  ...
  setEnabled(enabled: boolean): void {
    ...
    logger.info('[InteractionLogger] Logging toggled', { enabled });
  }
