...
type PersonaDetectResult = {
  persona: string | null;
  confidence: number;
};

type PersonaSwitchDecision = PersonaDetectResult & {
  reason: string;
...
export async function detectPersona(
   ...
   */
  function detectByKeywords(content: string): PersonaDetectResult {
    let bestMatch = null;
    let bestScore = 0;
   ...
   */
  async function detectByRAG(content: string, ai: any, guildId: string): Promise<PersonaDetectResult> {
    try {
    ...
    // IMPROVEMENT: Check which personas are mentioned in user's message
      const userMentionedPersonas = new Set<string>();
    ...
    const scoreRatio = totalScore > 0 ? bestScore / totalScore : 0;
      const personaHitCount = ragResult.data.hits.filter((hit: any) =>
 ...
 */
async function detectByAIAgent(
  messageContent: string,
  currentPersona: string | null,
  services: any
): Promise<PersonaDetectResult> {
  try {
...
Begin analysis:`;
    ...
    const aiConfidence = confidenceMatch?.[1] ? parseFloat(confidenceMatch[1]) : 0.5;
 ...
 */
const activeConversations = new Map<string, { personaName: string; expiresAt: number }>(); // Map<userId_channelId, {personaName, expiresAt}>
const CONVERSATION_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes

export function getActivePersona(userId: string, channelId: string): string | null {
...
export function setActivePersona(userId: string, channelId: string, personaName: string) {
  const key = `${userId}_${channelId}`;
...
export function clearActivePersona(userId: string, channelId: string) {
  const key = `${userId}_${channelId}`;
  activeConversations.delete(key);
