 ...
 */
export async function handleGameAccept(interaction: any) {
  try {
    ...
    logger.error("[GAME] Accept handler error", { error: getErrorMessage(error) });
    await interaction.reply({
 ...
 */
export async function handleGameDecline(interaction: any) {
  try {
    ...
    logger.error("[GAME] Decline handler error", { error: getErrorMessage(error) });
  }
}
 ...
 */
export async function handleGameAnswer(interaction: any) {
  try {
    ...
    logger.error("[GAME] Answer handler error", { error: getErrorMessage(error) });
  }
}
 ...
 * Start trivia game
 */
async function startTriviaGame(interaction: any, session: any) {
  ...
  const buttons = new ActionRowBuilder().addComponents(
    ...(session.gameData.answers as any[]).map((answer: string, idx: number) => {
 ...
 * Start riddle game
 */
async function startRiddleGame(interaction: any, session: any) {
  const embed = new EmbedBuilder()
    .setColor("#9B59B6")
  ...
  const filter = (m: any) => m.author.id === session.challengerId;
 ...
 * Start reaction game
 */
async function startReactionGame(interaction: any, session: any) {
  await interaction.update({
 ...
 */
async function startNumberGuessGame(interaction: any, session: any) {
  const embed = new EmbedBuilder()
    .setColor("#E67E22")
  ...
  const filter = (m: any) => m.author.id === session.challengerId && !isNaN(m.content);
  const collector = interaction.channel.createMessageCollector({
    filter,
...
export async function startGame({
  ...
  try {
    ...
    logger.error("[GAME] startGame failed", { error: getErrorMessage(error) });
    return {
      ok: false,
      error: {
        code: "DB_ERROR",
...
export async function handleClick({
  ...
  try {
    ...
    const cooldowns = getCollection<GameCooldownDoc>("game_cooldowns");
    const lastWin = await cooldowns.findOne({
      _id: cooldownKey,
    });
    ...
    // Award points
    const { award, getProfile } = await import("../services/points.js");
    const before = await getProfile(guildId, userId);
    const beforeLevel = before.ok ? Number(before.data?.level || 0) : 0;

    const pointsResult = await award(guildId, userId, 10);
    const leveledUp = pointsResult.ok
      ? pointsResult.data.level > beforeLevel
      : false;
    const pointsData = pointsResult.ok
      ? {
          awarded: 10,
      ...
      : { awarded: 10, points: 10, level: 1, leveledUp: false };
    ...
    logger.error("[GAME] handleClick failed", { error: getErrorMessage(error) });
    return {
      ok: false,
      error: {
        code: "UNKNOWN",
 ...
 */
export async function startDmSession({
  userId,
  guildId,
  personaName,
}: {
  ...
  try {
    ...
    logger.error("[GAME] startDmSession failed", { error: getErrorMessage(error) });
    return {
      ok: false,
      error: {
        code: "DB_ERROR",
 ...
 */
export async function answerDm({ userId, answer }: { userId: string; answer: string }) {
  try {
    ...
    logger.error("[GAME] answerDm failed", { error: getErrorMessage(error) });
    return {
      ok: false,
      error: {
        code: "DB_ERROR",
 ...
 * Get DM game status
 */
export async function getDmStatus(userId: string) {
  try {
    const session = await getCollection("dm_game_sessions").findOne({
      userId,
      status: "active",
    ...
    logger.error("[GAME] getDmStatus failed", { error: getErrorMessage(error) });
    return {
      ok: false,
      error: {
        code: "DB_ERROR",
 ...
 */
export async function cancelDm(userId: string) {
  try {
    ...
    logger.error("[GAME] cancelDm failed", { error: getErrorMessage(error) });
    return {
      ok: false,
      error: {
        code: "DB_ERROR",
