...
import { listPersonas } from "../services/persona.js";
 ...
 */
export async function handleAutoMemeDrop(client: any, services: any) {
  try {
    ...
    for (const guild of guilds) {
      try {
        // Get random media
        const mediaCol = getCollection<MediaDoc>("media");
        ...
        if (media.type === "image" && services.ai?.vlm && services.ai?.llm) {
          try {
    ...
    logger.error("[PROACTIVE] Auto meme drop failed", { error: getErrorMessage(error) });
  }
}
 ...
 */
export async function handleAutoPersonaChat(client: any, services: any) {
  try {
    logger.info("[PROACTIVE] Starting auto persona chat");

    if (!services.ai?.llm) {
      logger.warn("[PROACTIVE] LLM service not available");
      return;
    }
    ...
    for (const guild of guilds) {
      try {
        ...
        if (typeof targetChannel !== "string" || !targetChannel) continue;
        ...
        if (services.ai?.rag) {
          try {
            ...
            if (ragResult.ok && ragResult.data.hits?.length > 0) {
              ...
              if (randomHit?.chunk) topic = randomHit.chunk.substring(0, 200);
            }
          } catch (error) {
        ...
        // Persona: Style the message (if available)
        let finalMessage = conversationStarter;
        if (services.ai?.persona?.compose) {
          const personaResult = await services.ai.persona.compose(
            conversationStarter,
            randomPersona
          );
          if (personaResult.ok && personaResult.data?.text) {
            finalMessage = personaResult.data.text;
          }
        }
 ...
 */
export async function handleAutoMiniGame(client: any, services: any) {
  try {
    ...
    for (const guild of guilds) {
      try {
        ...
        let triviaContext: string | null = null;
        if (services.ai?.rag) {
          try {
            const ragResult = await services.ai.rag.search({
            ...
            if (ragResult.ok && ragResult.data.hits?.length > 0) {
              const firstHit = ragResult.data.hits[0];
              if (firstHit?.chunk) triviaContext = firstHit.chunk;
            }
          } catch (error) {
    ...
    logger.error("[PROACTIVE] Auto mini game failed", { error: getErrorMessage(error) });
  }
}
 ...
 */
export async function handleAutoStoryWeave(client: any, services: any) {
  try {
    ...
    for (const guild of guilds) {
      try {
        ...
        let storyteller = personas.data.find((p: any) => p.name === "Elio");
        if (!storyteller) {
          storyteller =
            personas.data[Math.floor(Math.random() * personas.data.length)];
        }
 ...
 */
export async function handleAutoWorldBuilder(client: any, services: any) {
  try {
    ...
    for (const guild of guilds) {
      try {
        ...
        const randomTopic = loreTopics[Math.floor(Math.random() * loreTopics.length)];
        ...
        // LLM: Format as interesting lore post (optional)
        const llmResult = services.ai?.llm
          ? await services.ai.llm.generate({
        ...
        const loreContent =
          llmResult?.ok && llmResult.data.text ? llmResult.data.text : ragResult.data.answer;

        // Select lore keeper persona (prefer Glordon for structured info)
        const personas = await listPersonas();
        if (!personas.ok || personas.data.length === 0) continue;

        let loreKeeper = personas.data.find((p: any) => p.name === "Glordon");
        if (!loreKeeper) {
          loreKeeper = personas.data[Math.floor(Math.random() * personas.data.length)];
        }
        if (!loreKeeper) continue;
 ...
 */
async function getEnabledGuilds(featureName: string): Promise<EnabledGuild[]> {
  try {
    const configCol = getCollection<GuildConfigDoc>("guild_config");
    const query: Record<string, unknown> = {
      [`proactive.${featureName}`]: true,
    };

    const docs = await configCol.find(query as any).toArray();
