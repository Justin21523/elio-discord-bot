 ...
 */
export function createFeedbackButtons(interactionId: string) {
 ...
 */
export async function handleThumbsUp(interaction: ButtonInteraction) {
  try {
    ...
    if (result.ok) {
      ...
      await interaction.update({
        components: [createDisabledFeedbackRow("up").toJSON()],
      });
    ...
    logger.error("[FEEDBACK] Thumbs up error", {
 ...
 */
export async function handleThumbsDown(interaction: ButtonInteraction) {
  try {
    ...
    if (result.ok) {
      // Update button to show feedback received
      await interaction.update({
        components: [createDisabledFeedbackRow("down").toJSON()],
      });
    ...
    logger.error("[FEEDBACK] Thumbs down error", {
      error: error instanceof Error ? error.message : String(error),
    });
    try {
      await interaction.reply({
 ...
 */
async function updateBanditFromFeedback(
  ...
  try {
    ...
    try {
      await ai.hybrid.updateBandit(strategy, reward);
      ...
      logger.warn("[FEEDBACK] Bandit update failed (non-critical)", {
        error: banditError instanceof Error ? banditError.message : String(banditError),
        strategy
      });
    }
  } catch (error) {
    logger.error("[FEEDBACK] Error updating bandit", {
      error: error instanceof Error ? error.message : String(error),
    });
  }
}
 ...
 */
function createDisabledFeedbackRow(selected: "up" | "down") {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
 ...
 */
export function isFeedbackButton(customId: string): boolean {
  return (
