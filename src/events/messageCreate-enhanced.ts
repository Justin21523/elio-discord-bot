...
export async function execute(message: any, services: any) {
  ...
  try {
    ...
    // Get persona
    let persona = await services.personas.get(decision.persona);
    if (!persona) {
    ...
    try {
      ...
      console.error('[ERR] RAG query failed:', getErrorMessage(error));
    }

    // Generate response with persona
    const conversationContext = history.slice(-10).map((h: any) => ({
      role: h.role,
      content: h.content
    }));
