...
export async function execute(interaction: any) {
  ...
  try {
    ...
    if (sub === 'set') {
      ...
      // Arm
      const armed: any = await Scheduler.arm({ guildId: String(guildId), channelId: String(channel.id), kind, hhmm } as any);
    ...
    if (sub === 'list') {
      ...
      const desc = rows
        .map((r: any) => `• **${r.kind}** → <#${r.channelId}> at \`${r.hhmm} UTC\`  (${r.enabled ? 'enabled' : 'disabled'})`)
        .join('\n');
    ...
    if (sub === 'reload') {
      ...
      await safeEdit(interaction, { content: `♻️ Reloaded schedules for this guild (armed: ${res.data?.armed ?? 0}).` });
