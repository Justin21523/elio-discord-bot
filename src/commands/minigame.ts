...
export async function execute(interaction: any, services: any) {
  ...
  try {
    ...
    logger.error("[MINIGAME_CMD] Error:", { error: msg });
...
async function handleStart(interaction: any, services: any) {
...
async function handleStop(interaction: any, services: any) {
...
async function handleStats(interaction: any, services: any) {
  await interaction.deferReply();

  const targetUser = interaction.options.getUser("user") || interaction.user;
  const result: any = await GameManager.getUserStats(targetUser.id);
  ...
  for (const [gameType, gameStats] of Object.entries(
    stats.byType as Record<string, any>
  )) {
    fields.push({
...
async function handleHelp(interaction: any, services: any) {
...
async function handleRecommend(interaction: any) {
  await interaction.deferReply({ ephemeral: true });
  let recs: any;
  let strategy = "local";
  ...
  const lines = (recs as any[]).map((r: any, idx: number) => {
  ...
  await interaction.editReply({
    ...
    components: [
      {
        type: 1,
        components: (recs as any[]).slice(0, 3).map((r: any) => ({
          type: 2,
          style: 1,
 ...
 */
function getDefaultRecommendations(): Array<{ game: string; score: number; reason: string }> {
  return [
    {
      game: "trivia",
      score: 5.0,
...
function prettyName(game: string): string {
...
async function handleGuess(interaction: any, services: any) {
...
async function handleClue(interaction: any, services: any) {
  await interaction.deferReply(); // Game actions are public
...
async function handleAnswer(interaction: any, services: any) {
  await interaction.deferReply(); // Game actions are public
...
async function handleDocQuery(interaction: any, services: any) {
  await interaction.deferReply(); // Game actions are public
  const query = interaction.options.getString("query");
  const game = GameManager.getGame(interaction.channel.id);
...
async function handleDocAnswer(interaction: any, services: any) {
  await interaction.deferReply(); // Game actions are public
  const text = interaction.options.getString("text");
  const game = GameManager.getGame(interaction.channel.id);
...
async function handleNarrate(interaction: any, services: any) {
  await interaction.deferReply(); // Game actions are public
...
async function handlePMI(interaction: any, services: any) {
  await interaction.deferReply(); // Game actions are public
...
async function handlePMIChoice(interaction: any, services: any) {
  await interaction.deferReply(); // Game actions are public
...
async function handleNext(interaction: any, services: any) {
  await interaction.deferReply(); // Game actions are public
  const game = GameManager.getGame(interaction.channel.id);
