...
export async function execute(interaction, services) {
  ...
  try {
    ...
    logger.error("[CONFIG] Social media config error", { error: error.message });
 ...
 */
function findPatternMatchedChannels(client, config) {
 ...
 */
async function handleSetFrequency(interaction) {
 ...
 */
async function handleToggleSource(interaction) {
  ...
  await interaction.editReply({
    content: `**Source Updated**\n\n${sourceNames[source]}: **${enabled ? "Enabled" : "Disabled"}**`
  });
...
async function handleRunNow(interaction) {
  ...
  try {
    ...
    logger.error("[CONFIG] Run now failed", { error: error.message });
    await interaction.followUp({
...
async function handleRemoveChannel(interaction) {
  ...
  const channelIds = config.channelIds.filter(id => id !== channel.id);
 ...
 */
async function handleSetChannelPattern(interaction) {
 ...
 */
async function handleClearChannelPattern(interaction) {
  const configCol = getCollection("bot_config");
  await configCol.updateOne(
    { key: CONFIG_KEY },
    {
 ...
 */
async function handleSetServerMode(interaction) {
...
async function handleRemoveServer(interaction) {
  ...
  const serverIds = config.serverIds.filter(id => id !== serverId);
 ...
 */
async function handleAddChannelById(interaction) {
 ...
 */
async function handleListAllChannels(interaction) {
  const configCol = getCollection("bot_config");
  const config = await configCol.findOne({ key: CONFIG_KEY });
