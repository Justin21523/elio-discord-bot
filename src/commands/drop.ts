 ...
 */
async function handleDropSet(interaction: any) {
  try {
    ...
    const armResult: any = await arm({
      ...
      handler: async ({
        ...
        const result: any = await executeDrop({
 ...
 * Handle /drop disable
 */
async function handleDropDisable(interaction: any) {
  try {
    const guildId = interaction.guildId;
