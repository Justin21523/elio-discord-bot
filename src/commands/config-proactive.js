...
async function handleView(interaction) {
  ...
  if (memeConfig) {
    ...
    if (memeConfig.channelIds?.length > 0) {
      response += `   ${memeConfig.channelIds.slice(0, 5).map(id => `<#${id}>`).join(", ")}${memeConfig.channelIds.length > 5 ? ` +${memeConfig.channelIds.length - 5} more` : ""}\n`;
    }
  ...
  if (personaConfig) {
    ...
    if (personaConfig.channelIds?.length > 0) {
      response += `   ${personaConfig.channelIds.slice(0, 5).map(id => `<#${id}>`).join(", ")}${personaConfig.channelIds.length > 5 ? ` +${personaConfig.channelIds.length - 5} more` : ""}\n`;
    }
  ...
  if (gameConfig) {
    ...
    if (gameConfig.channelIds?.length > 0) {
      response += `   ${gameConfig.channelIds.slice(0, 5).map(id => `<#${id}>`).join(", ")}${gameConfig.channelIds.length > 5 ? ` +${gameConfig.channelIds.length - 5} more` : ""}\n`;
    }
 ...
 */
function extractChannelIds(interaction, autoDetectPattern = null, includeServers = null, excludeServers = null) {
  const channelIds = [];
...
async function handleMemeDrop(interaction) {
  ...
  if (updateDoc.channelIds && updateDoc.channelIds.length > 0) {
    ...
    response += `${updateDoc.channelIds.slice(0, 5).map(id => `<#${id}>`).join(", ")}${updateDoc.channelIds.length > 5 ? ` +${updateDoc.channelIds.length - 5} more` : ""}\n`;
  }
  if (memeType) {
...
async function handleDropNow(interaction) {
  ...
  try {
    ...
    logger.error("[CONFIG] Drop now failed", { error: error.message });
    await interaction.followUp({
      content: `Failed to drop meme: ${error.message}`,
      ephemeral: true
    });
  }
}
...
async function handleSet(interaction) {
  ...
  const includeServers = includeServersStr ? includeServersStr.split(",").map(s => s.trim()).filter(Boolean) : null;
  const excludeServers = excludeServersStr ? excludeServersStr.split(",").map(s => s.trim()).filter(Boolean) : null;
  ...
  if (selectedChannels.length > 0) {
    ...
    channelSource = `selected: ${selectedChannels.map(ch => `<#${ch.id}>`).join(", ")}`;
  ...
  const featureNames = { meme: "Meme Drop", game: "Mini Game", persona: "Persona Chat" };
  const repeatNames = { daily: "Daily", "6h": "Every 6 hours", "4h": "Every 4 hours", "2h": "Every 2 hours", "1h": "Hourly" };
  const tzNames = {
    "8": "CST", "9": "JST/KST", "-8": "PST", "-7": "MST", "-6": "CST-US", "-5": "EST",
    "0": "GMT", "1": "CET", "2": "EET", "3": "MSK", "10": "AEST"
  };

  let response = `**${featureNames[feature]} Scheduled**\n\n`;
 ...
 */
function scheduleProactiveJob(client, schedule) {
  ...
  const job = cron.schedule(cronExpr, async () => {
    ...
    try {
      ...
      logger.error(`[CRON] Scheduled ${feature} failed`, { scheduleId, error: error.message });
    }
  });
 ...
 */
async function handleSchedules(interaction) {
  const schedulesCol = getCollection("proactive_schedules");
  ...
  const featureNames = { meme: "Meme Drop", game: "Mini Game", persona: "Persona Chat" };
  const repeatNames = { daily: "Daily", "6h": "Every 6h", "4h": "Every 4h", "2h": "Every 2h", "1h": "Hourly" };

  let response = `**Proactive Schedules** (${schedules.length} total)\n\n`;

  for (const schedule of schedules) {
    response += `**${featureNames[schedule.feature] || schedule.feature}**\n`;
    response += `  ID: \`${schedule.scheduleId}\`\n`;
    response += `  Time: ${schedule.timeStr} (${repeatNames[schedule.repeat] || schedule.repeat})\n`;
 ...
 */
async function handleDeleteSchedule(interaction) {
...
async function handleRunNow(interaction) {
  ...
  await interaction.editReply({
    content: `Running ${featureNames[feature]} now...`
  });
  ...
  try {
    ...
    await interaction.followUp({
      content: `${featureNames[feature]} completed!`,
      ephemeral: true
    });
  } catch (error) {
    logger.error("[CONFIG] Run now failed", { feature, error: error.message });
    await interaction.followUp({
      content: `${featureNames[feature]} failed: ${error.message}`,
      ephemeral: true
    });
  }
}
 ...
 */
export async function loadProactiveSchedules(client) {
  try {
    const schedulesCol = getCollection("proactive_schedules");
    const schedules = await schedulesCol.find({ enabled: true }).toArray();
    ...
    logger.error("[CONFIG] Failed to load proactive schedules", { error: error.message });
  }
}
