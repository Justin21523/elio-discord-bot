...
async function showOverview(interaction) {
  ...
  for (const [key, cat] of Object.entries(COMMAND_CATEGORIES)) {
    ...
    const preview = cat.commands.slice(0, 3).map((c) => `\`${c.name.split(" ")[0]}\``).join(", ");
    embed.addFields({
  ...
  embed.setFooter({
    text: `Elioverse Bot • ${Object.values(COMMAND_CATEGORIES).reduce((sum, cat) => sum + cat.commands.length, 0)} commands available`,
  });

  await interaction.reply({ embeds: [embed], ephemeral: true });
}

async function showCategoryHelp(interaction, categoryKey) {
