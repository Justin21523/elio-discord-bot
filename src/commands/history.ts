...
async function handleSync(interaction: any, db: any, aiClient: any) {
  ...
  const result: any = await syncSingleChannel(db, aiClient, channel, {
    maxDays: days,
...
async function handleStats(interaction: any, db: any) {
  await interaction.deferReply({ ephemeral: true });
...
async function handleSearch(interaction: any, db: any) {
  await interaction.deferReply({ ephemeral: true });
...
async function handleConfig(interaction: any, db: any) {
      ...
      .addFields(
        ...
        { name: 'Embeddings', value: historyConfig.generateEmbeddings ? 'Enabled' : 'Disabled', inline: true },
        {
          ...
          value: historyConfig.excludeChannels?.length
            ? historyConfig.excludeChannels.map((id: any) => `<#${id}>`).join(', ')
            : 'None',
        }
      )
      .setTimestamp();
...
async function handleExclude(interaction: any, db: any) {
  const channel = interaction.options.getChannel('channel');
...
async function handleInclude(interaction: any, db: any) {
  const channel = interaction.options.getChannel('channel');
...
async function handleActivity(interaction: any, db: any) {
  await interaction.deferReply({ ephemeral: true });
  ...
  const activityDays = (result.data as any[]) || [];
  const maxCount = Math.max(...activityDays.map((d: any) => d.messageCount));
  const chartLines = activityDays.map((day: any) => {
