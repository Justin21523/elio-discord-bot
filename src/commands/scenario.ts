 ...
 */
async function handleScenarioStart(interaction: any) {
  try {
    ...
    // Start session
    const result: any = await startSession({
      guildId,
      channelId,
      revealAfterMinutes,
    });
    ...
    const { sessionId, scenario, question, revealAt } = result.data as any;

    // Build option buttons
    const buttons = (scenario.options as any[]).map((opt: any, i: number) =>
      new ButtonBuilder()
 ...
 */
async function handleScenarioCancel(interaction: any) {
  try {
    const guildId = interaction.guildId;
    const channelId = interaction.channelId;
    ...
    // Cancel session
    const result: any = await cancel({ guildId, channelId });
 ...
 */
async function handleScenarioReveal(interaction: any) {
  try {
    ...
    // Reveal results
    const result: any = await reveal({ sessionId } as any);
 ...
 */
async function handleScenarioStats(interaction: any) {
  try {
    const sessionIdStr = interaction.options.getString("session-id", true);
    ...
    // Get stats
    const result: any = await getSessionStats({ sessionId } as any);
    ...
    if (stats.revealed) {
      ...
      if (stats.answerDistribution) {
        description += `\n**Distribution:**\n`;
        stats.answerDistribution.forEach((count: any, i: number) => {
 ...
 * @returns {Promise<{ok: boolean, data?: any, error?: any}>}
 */
export async function handleScenarioAnswer(interaction: any) {
  try {
    ...
    // Record answer
    const result: any = await answer({ sessionId, userId, answerIndex } as any);
