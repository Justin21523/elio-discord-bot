...
export async function execute(interaction: any, services: any) {
  ...
  try {
    if (subcommand === "start") {
      ...
      const datasetPath = (DATASET_PATHS as Record<string, string>)[String(dataset)];
      if (!datasetPath) {
      ...
      const result: any = await getJobStatus(jobId);
      ...
      const result: any = await listJobs({
        status: filter,
        limit: 10,
      });
      ...
      const { jobs, total } = result.data as any;
      ...
      const result: any = await cancelJob(jobId);
      ...
      const result: any = await prepareDataset({
        rawDataPath: dataset,
