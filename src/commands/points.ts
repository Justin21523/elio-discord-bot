...
export async function execute(interaction: any) {
  ...
  try {
    ...
    if (sub === 'show') {
      ...
      const res: any = await Points.getProfile(guildId, target.id);
    ...
    if (sub === 'add') {
      ...
      const res: any = await Points.award(guildId, target.id, delta);
    ...
    if (sub === 'set') {
      ...
      const res: any = await Points.setPoints(guildId, target.id, points);
    ...
    if (sub === 'reset-season') {
      const res: any = await Points.seasonalReset(guildId);
